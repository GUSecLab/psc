...
func main() {
	...
	y := suite.Point().Mul(x, nil)
        ...
        check(err)
        err = ioutil.WriteFile("private/dp" + strconv.Itoa(i + 1) + ".priv", out, 0644)
        check(err)
        fmt.Printf("%q \n",x.String())
        ...
        check(err)
        err = ioutil.WriteFile("public/dp" + strconv.Itoa(i + 1) + ".pub", out, 0644)
        check(err)
        fmt.Printf("%q \n",y.String())

        in, err := ioutil.ReadFile("private/dp" + strconv.Itoa(i + 1) + ".priv")
        check(err)
        ...
        in, err = ioutil.ReadFile("public/dp" + strconv.Itoa(i + 1) + ".pub")
        check(err)
        pub1 := &Schnorrkey.Pub{}
